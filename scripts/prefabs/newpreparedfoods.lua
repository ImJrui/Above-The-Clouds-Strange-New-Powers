...
local function MakePreparedFood(data)
    ...
    if spicename ~= nil then
        table.insert(foodassets, Asset("ANIM", "anim/newspices.zip"))
    ...
    local function fn()
        ...
        if spicename ~= nil then
            ...
            inst.AnimState:OverrideSymbol("swap_garnish", "newspices", spicename)
			...
			food_symbol_build = data.overridebuild or "new_cook_pot_food"
            inst.AnimState:PlayAnimation("idle")
            inst.AnimState:OverrideSymbol("swap_food", data.overridebuild or "new_cook_pot_food", data.basename or data.name)
            inst.scrapbook_overridedata = {"swap_food", data.overridebuild or "new_cook_pot_food", data.basename or data.name}
        else
			inst.AnimState:SetBuild(data.overridebuild or "new_cook_pot_food")
			inst.AnimState:SetBank("new_cook_pot_food")
            inst.AnimState:PlayAnimation("idle")
            inst.AnimState:OverrideSymbol("sweetandsourcarp", data.overridebuild or "new_cook_pot_food", data.basename or data.name)
            inst.scrapbook_overridedata = {"sweetandsourcarp", data.overridebuild or "new_cook_pot_food", data.basename or data.name}
        end
...
for k, v in pairs(require("features/newpreparedfoods_warly")) do
    table.insert(prefs, MakePreparedFood(v))
end


for k, v in pairs(require("features/newspicedfoods")) do
    table.insert(prefs, MakePreparedFood(v))
end
