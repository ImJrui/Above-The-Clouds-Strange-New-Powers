...
local COLLAPSIBLE_TAGS = { "NPC_workable","pickable" }
...
local function DoAOEWork(inst, x, z, isocean)
	for i, v in ipairs(TheSim:FindEntities(x, 0, z, inst.ammo_def.radius, nil, NON_COLLAPSIBLE_TAGS, COLLAPSIBLE_TAGS)) do
		if v:IsValid() and not v:IsInLimbo() then
			...
			then
				if v.components.shearable and v.components.shearable:CanShear() then
					v.components.shearable:Shear(inst, 1)
				elseif v.components.hackable and v.components.hackable:CanBeWorked() then
					v.components.hackable:WorkedBy(inst, v.components.hackable:GetWorkLeft())
				end
				...
				if v.components.workable then
					...
					isworkable = not (work_action == ACTIONS.DIG) and
...
local function DoInteriorQuake(inst)
...
local function OnHit_infused(inst, attacker, target)
    --give the value of target position to inst 把弹药的坐标设置在目标上，这样产生的爆炸发生在目标的位置而不是原来弹药的位置
    local x, y, z = target.Transform:GetWorldPosition()
    inst.Transform:SetPosition(x, y, z)
	inst.SoundEmitter:KillSound("toss")

	DoAOEDamage(inst, attacker, target, inst.ammo_def.explosivedamage, inst.ammo_def.radius)
    DoAOEWork(inst, x, z, false)
	DoInteriorQuake(inst)
...
local function onloadammo_infused(inst, data)
	if data ~= nil and data.slingshot then
		data.slingshot:AddTag("PLREBALANCE_SLINGSHOTPOINTTARGET")
	end
end

local function onunloadammo_infused(inst, data)
	if data ~= nil and data.slingshot then
		data.slingshot:RemoveTag("PLREBALANCE_SLINGSHOTPOINTTARGET")
	end
end

local function OnHit_lightning(inst, attacker, target)
    if target ~= nil and target:IsValid() and attacker ~= nil and attacker:IsValid() then
...
--------------------------------------------------------------------------
local ALLOY_TAGS = { "alloyammo" }
local ALLOY_NOTAGS = { "INLIMBO" }

local function FindStackableAlloyAmmo(inst, radius)
	local x, y, z = inst.Transform:GetWorldPosition()
	local num = inst.components.stackable:StackSize()
	for i, v in ipairs(TheSim:FindEntities(x, y, z, radius, ALLOY_TAGS, ALLOY_NOTAGS)) do
		if v ~= inst and v.components.inventoryitem.is_landed and v.components.stackable:RoomLeft() >= num then
			return v
		end
	end
...
local function OnLanded2_Alloy(inst) --this is the inv item
	inst:RemoveEventCallback("on_landed", OnLanded2_Alloy)

	local other = FindStackableAlloyAmmo(inst, 0.25)
	if other then
		other.components.stackable:Put(inst)
	end
end

local function OnLanded_Alloy(inst) --this is the inv item
	inst:RemoveEventCallback("on_landed", OnLanded_Alloy)

	local other = FindStackableAlloyAmmo(inst, 0.5)
	if other then
		local vx, vy, vz = inst.Physics:GetVelocity()
		other.components.stackable:Put(inst)
		if vx ~= 0 or vz ~= 0 then
			...
			Launch2(other, other, speed * 0.5, 0.1, 0, 0, 3, dir - 10 + math.random() * 20)
			other.components.inventoryitem:SetLanded(false, true)
			other:ListenForEvent("on_landed", OnLanded2_Alloy)
		end
	end
end

local function OnHit_Alloy(inst, attacker, target)
	if attacker and attacker:IsValid() and
		attacker.components.skilltreeupdater and attacker.components.skilltreeupdater:IsActivated("walter_ammo_lucky")
	then
		if target and target:IsValid() then
			StartFlash(inst, target, 1, 0, 0)
			if math.random() < TUNING.SLINGSHOT_AMMO_DREADSTONE_RECOVER_CHANCE then
				local ammo = SpawnPrefab("slingshotammo_alloy")
				LaunchAt(ammo, target, attacker and attacker:IsValid() and attacker or nil, 1, 1, target:GetPhysicsRadius(0), 40)
				ammo.components.inventoryitem:SetLanded(false, true)
				ammo:ListenForEvent("on_landed", OnLanded_Alloy)
			end
		end
	end
end
...
local function inv_fn(ammo_def)
	...
	if ammo_def.symbol then
		inst.scrapbook_overridedata = { "rock", "newslingshotammo", ammo_def.symbol }
	end
...
{
    {
		name = "slingshotammo_infused",
        symbol = "infused",
		damage = 68,
        onhit = OnHit_infused,
        onhitnoncombat = OnHit_infused,
		onloadammo = onloadammo_infused,
		onunloadammo = onunloadammo_infused,
		explosivedamage = 34,
		radius = 3.75,
		skill = "walter_ammo_lunar",
	},
	{
		name = "slingshotammo_alloy",
		symbol = "alloy",
