...
NEW_VEGGIES =
{
    radish = MakeVegStats(COMMON,   TUNING.CALORIES_SMALL,  TUNING.HEALING_TINY,    TUNING.PERISH_MED, 0,
                                    TUNING.CALORIES_SMALL,  TUNING.HEALING_SMALL,   TUNING.PERISH_FAST, 0,
                                    {"med", 0.05, 0.8},    {"small", 0.1, nil},
									nil, nil,
                                    {prefab = "radish"}),

    aloe = MakeVegStats(COMMON,     9.375, 8, TUNING.PERISH_MED, 0,
                                    12.5, 3, TUNING.PERISH_FAST, 0,
                                    {"med", 0.05, 0.8}, {"small", 0.1, nil},
									nil, nil,
                                    {prefab = "aloe"})
}

for veggie in pairs(NEW_VEGGIES) do
    VEGGIES[veggie] = NEW_VEGGIES[veggie]
end
...
local function dowaxfn(inst, doer, waxitem)
    ...
    if doer.components.inventory and doer.components.inventory:IsHeavyLifting() and doer.components.inventory:GetEquippedItem(EQUIPSLOTS.BODY) == inst then
        ...
        waxedveggie.AnimState:PushAnimation("crop_oversized")
    end
    inst:Remove()
    return true
end
...
local function MakeVeggie(name, has_seeds)
	...
	if NEW_VEGGIES[name].lure_data ~= nil then
		table.insert(assets, Asset("ANIM", "anim/"..NEW_VEGGIES[name].lure_data.build..".zip"))
	end
	...
	local dryable = NEW_VEGGIES[name].dryable
    ...
    local function fn()
        ...
        if NEW_VEGGIES[name].extra_tags_fresh then
            for _, extra_tag in ipairs(NEW_VEGGIES[name].extra_tags_fresh) do
                inst:AddTag(extra_tag)
            end
        end

        local float = NEW_VEGGIES[name].float_settings
        if float ~= nil then
        ...
        inst:AddComponent("edible")
        inst.components.edible.healthvalue = NEW_VEGGIES[name].health
        inst.components.edible.hungervalue = NEW_VEGGIES[name].hunger
        inst.components.edible.sanityvalue = NEW_VEGGIES[name].sanity or 0
        inst.components.edible.foodtype = FOODTYPE.VEGGIE
        inst.components.edible.secondaryfoodtype = NEW_VEGGIES[name].secondary_foodtype

        inst:AddComponent("perishable")
        inst.components.perishable:SetPerishTime(NEW_VEGGIES[name].perishtime)
        inst.components.perishable:StartPerishing()
        inst.components.perishable.onperishreplacement = "spoiled_food"
        ...
        -- Regular NEW_VEGGIES are weighable but don't have a weight. They all show the same
        -- result when put in a trophyscale_oversizedveggies, and always replace other
        -- regular NEW_VEGGIES when attempting to do so.
        if not SEEDLESS[name] then
		...
		if NEW_VEGGIES[name].lure_data ~= nil then
			inst:AddComponent("oceanfishingtackle")
			inst.components.oceanfishingtackle:SetupLure(NEW_VEGGIES[name].lure_data)
		end

		local halloweenmoonmutable_settings = NEW_VEGGIES[name].halloweenmoonmutable_settings
    ...
    local function fn_cooked()
        ...
        if NEW_VEGGIES[name].extra_tags_cooked then
            for _, extra_tag in ipairs(NEW_VEGGIES[name].extra_tags_cooked) do
                inst:AddTag(extra_tag)
            end
        end

        local float = NEW_VEGGIES[name].cooked_float_settings
        if float ~= nil then
            MakeInventoryFloatable(inst, float[1], float[2], float[3])
        else
            MakeInventoryFloatable(inst)
        ...
        inst:AddComponent("perishable")
        inst.components.perishable:SetPerishTime(NEW_VEGGIES[name].cooked_perishtime)
        inst.components.perishable:StartPerishing()
        inst.components.perishable.onperishreplacement = "spoiled_food"

        inst:AddComponent("edible")
        inst.components.edible.healthvalue = NEW_VEGGIES[name].cooked_health
        inst.components.edible.hungervalue = NEW_VEGGIES[name].cooked_hunger
        inst.components.edible.sanityvalue = NEW_VEGGIES[name].cooked_sanity or 0
        inst.components.edible.foodtype = FOODTYPE.VEGGIE
        inst.components.edible.secondaryfoodtype = NEW_VEGGIES[name].secondary_foodtype
    ...
    local function fn_oversized()
        ...
        inst:AddComponent("perishable")
        inst.components.perishable:SetPerishTime(NEW_VEGGIES[name].perishtime * OVERSIZED_PERISHTIME_MULT)
        inst.components.perishable:StartPerishing()
        ...
        --inst.components.symbolswapdata.build = plant_def.build
        --inst.components.symbolswapdata.symbol = plant_def.symbol
...
local prefs = {}
for veggiename,veggiedata in pairs(NEW_VEGGIES) do
